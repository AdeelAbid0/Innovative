...
export function CodeRainEffect() {
  ...
  useEffect(() => {
    ...
    const codeSnippets = [
      ...
      'npm', 'git', 'push', 'commit', 'merge', 'pull',
      'API', 'REST', 'JSON', 'HTTP', 'GET', 'POST',
      '=>', '{}', '[]', '()', '===', '!==', '&&', '||',
      '01', '10', '001', '101', '011', '100', '110',
      '<div>', '</div>', '</>',  'onClick', 'onChange',
      'true', 'false', 'null', 'undefined', 'void',
      'interface', 'type', 'enum', 'any', 'unknown'
    ];
    ...
    function draw() {
      ...
      for (let i = 0; i < drops.length; i++) {
        ...
        if (drop.y > 0 && drop.y < canvas.height) {
          ctx.fillStyle = `rgba(12, 228, 33, ${Math.min(drop.opacity * 2, 0.9)})`;
          ctx.fillText(drop.text, x, drop.y);
        }
