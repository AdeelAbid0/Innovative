...
export const templates: Template[] = [
  {
    ...
    description: 'A stunning portfolio template designed for freelancers, creatives, and professionals who want to showcase their work beautifully.',
    features: [
  ...
  },
  {
    ...
    description: 'Perfect landing page template for tech startups, SaaS products, and digital agencies looking to convert visitors into customers.',
    features: [
  ...
  },
  {
    ...
    description: 'Complete e-commerce solution with shopping cart, product pages, checkout flow, and admin dashboard.',
    features: [
  ...
  },
  {
    ...
    description: 'Professional corporate website template perfect for businesses, consulting firms, and enterprise companies.',
    features: [
  ...
  },
  {
    ...
    preview: 'https://images.unsplash.com/photo-1606983340126-99ab4feaa64a?w=800&h=600&fit=crop',
    tags: ['Photography', 'Studio', 'Gallery', 'Creative'],
    description: 'Stunning photography portfolio template with gallery, booking system, and client portal.',
    features: [
  ...
  },
  {
    ...
    description: 'Beautiful restaurant website with online menu, reservations, and food ordering system.',
    features: [
  ...
  },
  {
    ...
    description: 'Premium agency template showcasing services, portfolio, and team with stunning animations.',
    features: [
  ...
  },
  {
    ...
    preview: 'https://images.unsplash.com/photo-1486312338219-ce68d2c6f44d?w=800&h=600&fit=crop',
    tags: ['Blog', 'Magazine', 'News', 'Content'],
    description: 'Feature-rich blog and magazine template with multiple layouts and content management.',
    features: [
...
export function getFeaturedTemplates(): Template[] {
  return getAllTemplates().filter(template => template.isBestSeller || template.isNew).slice(0, 6);
}
...
export function updateTemplate(templateId: string, updatedTemplate: Partial<Template>): boolean {
  try {
    if (typeof window === 'undefined') return false;
    ...
    customTemplates[templateIndex] = { ...customTemplates[templateIndex], ...updatedTemplate };
...
export function getTemplateStats() {
  ...
  return {
    ...
    averagePrice: Math.round(allTemplates.reduce((sum, t) => sum + t.price, 0) / allTemplates.length),
    averageRating: Math.round((allTemplates.reduce((sum, t) => sum + t.rating, 0) / allTemplates.length) * 10) / 10
  };
}
...
export function validateTemplate(template: Partial<Template>): { valid: boolean; errors: string[] } {
  ...
  if (!template.price || template.price <= 0) errors.push('Valid price is required');
  if (!template.thumbnail?.trim()) errors.push('Thumbnail image is required');
  if (!template.features?.length) errors.push('At least one feature is required');
...
export function importTemplateData(data: string): boolean {
  try {
    ...
    if (parsed.customTemplates && Array.isArray(parsed.customTemplates)) {
      localStorage.setItem('customTemplates', JSON.stringify(parsed.customTemplates));
...
export function filterTemplates(templateList: Template[], filters: {
  category?: string;
  search?: string;
  ...
  if (filters.priceRange) {
    ...
    filtered = filtered.filter(template => 
      template.price >= minPrice && template.price <= maxPrice
    );
  }
